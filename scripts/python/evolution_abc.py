...
from classes import evolution_abc, swarmulator
...
if args.plot is not None:
    e.load(args.plot)
    e.plot_evolution('gggg.png')

# Resume evolution from file args.resume
elif args.resume is not None:
	e.load(args.resume)
	p = e.evolve(verbose=True, generations=args.gen, population=e.pop)

# Just run normally from the beginning
else:
    p = e.evolve(verbose=True, generations=args.gen)
